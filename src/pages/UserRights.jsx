...
const rightsData = [
        ...
        rights: [
            {
                ...
                roles: ["super_admin", "cr_admin", "teacher_admin", "student"],
            },
        ],
    },
    {
    ...
    },
    {
        ...
        icon: "👥",
        rights: [{ key: "assign_cr", label: "Assign / Remove CR", roles: ["super_admin", "cr_admin"] }],
    },
    {
        module: "Todo List",
        icon: "📋",
        rights: [
            { key: "view_todo_list", label: "View Todo List", roles: ["super_admin", "cr_admin", "student"] },
            {
                ...
                label: "Manage Todo List (Add / Edit / Delete)",
                roles: ["super_admin", "cr_admin", "student"],
            },
        ],
    },
    {
        ...
        rights: [
            {
                key: "view_class_schedule",
                label: "View Class Schedule",
                roles: ["super_admin", "cr_admin", "teacher_admin", "student"],
            },
            {
                key: "edit_class_schedule",
    ...
    },
    {
        ...
        rights: [
            ...
            {
                key: "approve_attendance",
                label: "Approve / Reject Attendance",
                roles: ["super_admin", "cr_admin", "teacher_admin"],
            },
        ],
    },
...
function UserRights() {
    ...
    const totalRights = role => rightsData.flatMap(m => m.rights).filter(r => roleHasRight(role, r.key)).length;
    ...
    const handleUserSave = async () => {
        ...
        if (entries.length === 0) {
            showToast("No user-level changes to save.", "info");
            return;
        }
        for (const [key, granted] of entries) {
        ...
        <div>
                ...
                {isAdminUser && (
                    <div style={{ marginTop: "14px", display: "flex", flexDirection: "column", gap: "12px" }}>
                        ...
                        <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
                            ...
                            <button className="btn btn-secondary btn-sm" onClick={handleDiscard} disabled={!isDirty}>
                                ↩ Discard
                            </button>
                            <button
                                className="btn btn-secondary btn-sm"
            ...
            <div className="panel" style={{ marginBottom: "25px" }}>
                ...
                <div className="panel-body" style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
                    ...
                    >
                        <div
                            className="student-avatar avatar-cr"
                            style={{ width: "52px", height: "52px", fontSize: "1.3rem" }}
                        >
                            🤝
                        </div>
                        <div style={{ flex: 1 }}>
                            <div style={{ fontSize: "0.95rem", fontWeight: "700", color: "var(--secondary)" }}>
                                CR Admin
                            </div>
                            <div style={{ fontSize: "0.8rem", color: "var(--gray)" }}>
                                {totalRights("cr_admin")} access rights
                            </div>
                        </div>
                        <div style={{ fontSize: "1.6rem", fontWeight: "800", color: "var(--secondary)" }}>
                            {totalRights("cr_admin")}
                        </div>
                    </div>
                    ...
                    >
                        <div
                            className="student-avatar avatar-teacher"
                            style={{ width: "52px", height: "52px", fontSize: "1.3rem" }}
                        >
                            👩‍🏫
                        </div>
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: "0.8rem", color: "var(--gray)" }}>
                                {totalRights("teacher_admin")} access rights
                            </div>
                        </div>
                        <div style={{ fontSize: "1.6rem", fontWeight: "800", color: "var(--accent)" }}>
                            {totalRights("teacher_admin")}
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                {filteredRights.length === 0 && (
                                    <tr>
                                        <td
                                            colSpan={5}
                                            style={{ textAlign: "center", padding: "24px", color: "var(--gray)" }}
                                        >
                                            No rights match "<strong>{searchTerm}</strong>". Try a different search or
                                        ...
                                        {group.rights.map(right => (
                                            <tr key={right.key}>
                                                ...
                                                {roles.map(role => {
                                                    ...
                                                    // Only let the super admin edit non-super-admin roles
                                                    const editable = isAdminUser && role !== "super_admin";
                                                    return (
                                                        <td
                                                            key={role}
                                                            style={{
                                                        ...
                                                        >
                                                            {editable ? (
                                                                <label
                                                                    ...
                                                                    title={
                                                                        granted
                                                                            ? "Click to remove this right"
                                                                            : "Click to grant this right"
                                                                    }
                                                                >
                                                                    <input
                                                                        type="checkbox"
                                                                        checked={granted}
                                                                        onChange={() =>
                                                                            toggleDraft(right, role, !granted)
                                                                        }
                                                                        style={{
                                                                            width: "18px",
                                                                            height: "18px",
            ...
            {!isAdminUser && (
                <p style={{ marginTop: "16px", fontSize: "0.8rem", color: "var(--gray)" }}>
                    You are viewing the rights matrix as <strong>{roleLabel(currentUser?.role)}</strong>. Only the Super
                    Admin can edit rights. Ticked changes are applied when the Super Admin clicks{" "}
...
function PerUserRightsSection({
    ...
    const saveU = async () => {
        ...
        for (const [k, g] of e) {
            const [rk, uid] = k.split(":");
            await setUserPermission(Number(uid), rk, g);
        }
        setUserDraft({});
                    ...
                    {selUser ? (
                        <>
                            ...
                            >
                                ...
                                >
                                    {Object.keys(userPermissions?.[selUser.id] || {}).length} active override
                                    {Object.keys(userPermissions?.[selUser.id] || {}).length !== 1 ? "s" : ""}
                                </span>
                            </div>
                            ...
                            {unsavedCount > 0 && (
                                ...
                                >
                                    ...
                                    <div style={{ marginLeft: "auto", display: "flex", gap: "8px" }}>
                                        <button className="btn btn-primary btn-sm" onClick={saveU}>
                                            💾 Save Overrides
                                        </button>
                                        <button
                                            className="btn btn-secondary btn-sm"
                            ...
                            >
                                <input
                                    ...
                                    onKeyDown={e => {
                                        if (e.key === "Enter") setRightSearch(rightSearchInput);
                                    }}
                                ...
                                {rightSearch && (
                                    <span style={{ fontSize: "0.75rem", color: "var(--gray)" }}>
                                        Showing {filteredRightsForUser.reduce((n, g) => n + g.rights.length, 0)} of{" "}
                                        {rightsData.reduce((n, g) => n + g.rights.length, 0)} permissions
                                    </span>
                                )}
                            </div>
                                    ...
                                    <tbody>
                                        {filteredRightsForUser.length === 0 && (
                                            <tr>
                                                ...
                                                >
                                                    No permissions match "<strong>{rightSearch}</strong>". Try a
                                                    different search or click <strong>Clear</strong>.
                                                </td>
                                            </tr>
                                        )}
                                                ...
                                                {g.rights.map(right => {
                                                    ...
                                                    return (
                                                        <tr
                                                            key={right.key}
                                                            style={
                                                                dr ? { background: "var(--dirty-row-bg)" } : undefined
                                                            }
                                                        >
                            ...
                            >
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        resetUserPermissions();
                                        setUserDraft({});
                                        showToast("All user overrides reset.", "success");
                                    }}
                                >
