...
function MyTeacherCard() {
  ...
  const handleUpload = async (file) => {
    ...
    setUploading(true);
    try {
      const res = await api.uploadTeacherCardPhoto(data.teacher.name, file);
  ...
  if (!data?.linked) {
    return (
      <div className="page-header">
        <h1>My Teacher ID Card</h1>
        <p>{data?.message || 'No teacher profile is linked to this account yet.'}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="page-header">
        <h1>My Teacher ID Card</h1>
        <p>This is your official staff identification card. An admin must approve your photo before it is issued.</p>
      </div>
            ...
            {card?.cardStatus === 'pending' && (
              <div className="issued-box issued-box-warning">
                ...
                <div>
                  ...
                  <span>An admin or CR will verify your photo before issuing the card.</span>
                </div>
              </div>
            )}
