...
export function initDatabase() {
  ...
  `);
    ...
    const teacherCardTables = db
        .prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='teacher_cards'`)
        .all();
    ...
    if (!existingTables.includes("attendance_geofences")) {
      ...
      createdAt TEXT NOT NULL
        ...
        const geoCols = db
            .prepare(`PRAGMA table_info(attendance_geofences)`)
            .all()
            .map(c => c.name);
...
function seedStudents(iS) {
    [
        ...
        [
            "Sneha Patel",
            "sneha.patel@school.edu",
            "+91 98776 55443",
            "STU-004",
            "BSIT",
            "Female",
            "234 River St Ahmedabad",
            "2008-12-05",
            "2020-06-01",
            "Active",
            0,
        ...
        [
            "Fatima Sheikh",
            "fatima.sheikh@school.edu",
            "+91 90099 88776",
            "STU-006",
            "BBA",
            "Female",
            "89 Rose Garden Hyderabad",
            "2010-07-30",
            "2021-06-01",
            "Active",
            0,
        ...
        [
            "Ananya Gupta",
            "ananya.gupta@school.edu",
            "+91 94455 66778",
            "STU-008",
            "BSCS",
            "Female",
            "678 Sunny St Lucknow",
            "2009-04-12",
            "2020-06-01",
            "Active",
            0,
        ...
        [
            "Rohan Joshi",
            "rohan.joshi@school.edu",
            "+91 93322 11009",
            "STU-009",
            "BSIT",
            "Male",
            "34 Mountain View Nashik",
            "2008-09-08",
            "2020-06-01",
            "Active",
            0,
        ...
        [
            "Kavya Nair",
            "kavya.nair@school.edu",
            "+91 91122 33445",
            "STU-010",
            "BSAF",
            "Female",
            "567 Marine Drive Kochi",
            "2010-02-20",
            "2021-06-01",
            "Active",
            0,
...
export function seedDatabase() {
    ...
    const iU = db.prepare(
        "INSERT INTO users (username,email,password,fullName,role,status,className,registrationDate) VALUES (?,?,?,?,?,?,?,?)",
    );
    const iS = db.prepare(
        "INSERT INTO students (name,email,phone,rollNo,className,gender,address,dateOfBirth,admissionDate,status,isCR) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
    );
    const iT = db.prepare(
        "INSERT INTO teachers (name,email,phone,subject,qualification,experience,className,joiningDate) VALUES (?,?,?,?,?,?,?,?)",
    );
    ...
    const iM = db.prepare(
        "INSERT INTO marks (studentId,studentName,subject,marks,grade,examType) VALUES (?,?,?,?,?,?)",
    );
    const iA = db.prepare(
    ...
    db.transaction(() => {
        ...
        seedMarks(iM);
        seedClassSchedules();
        seedAttendanceRecords(iA);
    })();
    console.log("Database seeded");
}

// ── Async-compatible facade ──────────────────────────────────────────────
// These thin wrappers let route handlers use the same await db.get/all/run()
// API that db.supabase.js exposes. On SQLite they resolve synchronously
// (Promise.resolve), so there is zero overhead in the SQLite path.
//
// NOTE: We intentionally do NOT override db.exec() — it is a native
// better-sqlite3 method used synchronously by initDatabase()/seedDatabase().
// Route handlers that need exec (permissions.js, index.js) call it directly;
// on SQLite it returns a sync value (await on non-Promise is harmless).

db.get = (sql, params = []) =>
  Promise.resolve(db.prepare(sql).get(...(Array.isArray(params) ? params : [params])));
db.all = (sql, params = []) =>
  Promise.resolve(db.prepare(sql).all(...(Array.isArray(params) ? params : [params])));
db.run = (sql, params = []) => {
...
// ── Async-compatible transaction wrapper ─────────────────────────────────
// Matches db.supabase.js's transaction({ run, get, all }) => { … } interface.
// Uses manual BEGIN/COMMIT/ROLLBACK so the callback can be async while all
// actual DB operations still execute synchronously within the same connection.
const _nativeExec = db.exec.bind(db);
db.transaction = (fn) => {
  return async (...args) => {
    _nativeExec('BEGIN');
    try {
      const result = await fn({ run: db.run, get: db.get, all: db.all }, ...args);
      _nativeExec('COMMIT');
      return result;
    } catch (err) {
      _nativeExec('ROLLBACK');
      throw err;
    }
  };
};
